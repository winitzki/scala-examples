...
class Chapter03_02_examplesSpec extends FlatSpec with Matchers {
  ...
  it should "ex01: define a disjunction type via case classes" in {
    ...
    def isThisSaturday(d: DayOfWeek): Boolean = ???
  ...
  it should "ex02: enhanced DayOfWeek" in {
    ...
    final case class Saturday(wakeUp: LocalDate) extends DayOfWeek
    ...
    def howMuchPaid(d: DayOfWeek): Option[Double] = ???
  ...
  def solve2: ((Double, Double)) ⇒ RootsOfQuadratic = {
    case (b, c) ⇒ // x^2 + b*x + c == 0
  ...
  def rootAverage(roots: RootsOfQuadratic): Option[Double] = ???
  ...
  it should "ex05: solve 100 random equations" in {
    def getRandom: Double = scala.util.Random.nextDouble() * 2 - 1

    val coeffs: Seq[(Double, Double)] = Seq.fill(100)((getRandom, getRandom))
  ...
  it should "ex06: implement fully generic function with option pairs" in {
    ...
    def f2[A, B]: ((Option[A], Option[B])) ⇒ Option[(A, B)] = {
      case (maybeA, maybeB) ⇒
        val result: Option[Option[(A, B)]] = maybeA.map(x ⇒ maybeB.map(y ⇒ (x, y)))
        result.flatten
    }
