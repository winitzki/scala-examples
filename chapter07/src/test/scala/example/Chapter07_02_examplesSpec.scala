...
class Chapter07_02_examplesSpec extends FlatSpec with FlattenableLawChecking with CatsLawChecking {
  ...
  behavior of "monad constructions"

  it should "verify the disjunction of Id and M" in {

    import cats.syntax.functor._
    import cats.syntax.flatMap._
    import cats.syntax.monad._

    def point[F[_] : Monad, A]: A ⇒ Either[A, F[A]] = x ⇒ Left(x)

    def join[F[_] : Monad : Functor, A]: Either[Either[A, F[A]], F[Either[A, F[A]]]] ⇒ Either[A, F[A]] = {
      case Left(x) ⇒ x // This could be either Left() or Right()!
      case Right(fma) ⇒ Right(fma.map {
        case Left(x) ⇒ Monad[F].pure(x)
        case Right(ma) ⇒ ma
      }.flatten)
    }
    ...
    def join[F, Either[A, F[A]]]: Either[Either[A, F[A]], F[Either[Either[A, F[A]], F[Either[A, F[A]]]]]] ⇒ Either[Either[A, F[A]], F[Either[A, F[A]]]]  = {
      case Left(x) ⇒ Left(x)
      case Right(fma) ⇒ Right(fma.map {
        case Left(x) ⇒ Monad[F].pure(x)
        case Right(ma) ⇒ ma
    ...
    join ◦ join : Either[Either[A, F[A]], F[Either[Either[A, F[A]], F[Either[A, F[A]]]]]] ⇒ Either[A, F[A]] =  {
      case Left(x) ⇒ Left(x)
      case Right(fma) ⇒
