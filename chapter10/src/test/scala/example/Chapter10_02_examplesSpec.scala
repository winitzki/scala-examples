...
class Chapter10_02_examplesSpec extends FlatSpec with Matchers {
  ...
  it should "define the tree encoding of free instance for any inductive typeclass" in {
    ...
    // The type definition of Free[C, Z] is recursive: `Free[C, Z] = Z + C[ Free[C, Z] ]`.
    ...
    implicit def freeIsC[C[_], Z]: C[Free[C, Z]] ⇒ Free[C, Z] = Ops.apply

    // Helper methods for working with Free[C, Z] values.
    def wrap[C[_], Z]: Z ⇒ Free[C, Z] = Wrap.apply

    def ops[C[_], Z](cf: C[Free[C, Z]]): Free[C, Z] = freeIsC(cf)
    ...
    // Create a value of this type and perform some operations.
  ...
  it should "define the Church/tree encoding of free instance for any inductive typeclass" in {
    ...
    trait CFreeP[C[_], Z, X] {
      def wrap: Z ⇒ X

      def ops: C[X] ⇒ X
    }

    // The type definition is not recursive but has a universally quantified type.
    trait CFree[C[_], Z] {
